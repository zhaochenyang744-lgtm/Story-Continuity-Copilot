"""Deterministic, temporary-runtime V6 candidate preflight.  It never calls a real Provider."""
...
from evaluation.v2_fixture_loader import V6_CORPUS_PATHS, fixture_runtime
from evaluation.validate_eval_set_v6_candidate import load_v6_candidate, validate_all


class DeterministicV6NoIssueProvider:
    """A local test double that only exercises the product API chain."""
    label = "evaluation-v6-deterministic-test-provider"
    model_label = "evaluation-v6-deterministic-test-model"
    available = True
...
def _data(response: Any) -> dict[str, Any]:
    if response.status_code >= 400:
        raise RuntimeError(f"v6_fixture_api_failed:{response.status_code}")
...
def preflight_v6_candidate(corpus_key: str | None = None) -> dict[str, Any]:
    validation = validate_all()
    if validation["lifecycle"] != "pre_run":
        raise RuntimeError("v6_candidate_preflight_refuses_completed_formal_run")
    if corpus_key is not None and corpus_key not in V6_CORPUS_PATHS:
        raise ValueError("unknown_v6_evaluation_fixture_corpus")
    cases = [case for case in load_v6_candidate()["cases"] if corpus_key is None or case["corpus_key"] == corpus_key]
    provider = DeterministicV6NoIssueProvider()
    rows: list[dict[str, Any]] = []
    for case in cases:
        with fixture_runtime(case["corpus_key"], provider, V6_CORPUS_PATHS) as runtime:
            ...
            if [project["id"] for project in projects] != [identity.project_id]:
                raise RuntimeError("v6_fixture_account_project_scope_invalid")
            ...
            if expected != lineage:
                raise RuntimeError(f"v6_fixture_lineage_not_parseable:{case['case_id']}")
            if not expected <= returned:
                raise RuntimeError(f"v6_fixture_expected_evidence_missed:{case['case_id']}")
            ...
            if counts != {"users": 1, "projects": 1, "runs": 1} or cross_project_evidence != 0:
                raise RuntimeError("v6_fixture_account_project_isolation_invalid")
    ...
    if len(rows) != len(cases) or provider.calls != len(rows):
        raise RuntimeError("v6_fixture_fake_provider_call_count_invalid")
    return {
        "schema_version": "scc-eval-v6-fixture-preflight-v1",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--corpus", choices=sorted(V6_CORPUS_PATHS))
    args = parser.parse_args()
    print(json.dumps(preflight_v6_candidate(args.corpus), ensure_ascii=False, indent=2))
